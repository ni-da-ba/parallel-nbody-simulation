...
void print_usage_if_needed(int rank) {
  if (rank == 0) {
    std::cout
        ...
        << "Defaults: mode=allpairs, N=3, steps=5, dt=0.01, theta=0.5, snapshot_interval=0, "
           "snapshot_prefix=snapshot\n\n";
  }
}

void save_snapshot_csv(const std::vector<Particle>& particles, int step,
  ...
  for (std::size_t i = 0; i < particles.size(); ++i) {
    ...
    out << i << ',' << p.position.x << ',' << p.position.y << ',' << p.position.z << ','
        << p.velocity.x << ',' << p.velocity.y << ',' << p.velocity.z << ',' << p.mass << '\n';
  }
}

void step_particles_barnes_hut_from_global(std::vector<Particle>& local_particles,
                                           const std::vector<Particle>& global_particles,
                                           int global_start_index, double dt, double theta) {
  ...
  for (std::size_t i = 0; i < local_particles.size(); ++i) {
    ...
    accelerations[i] =
        root.compute_acceleration_on_particle(global_index, global_particles, theta, SOFTENING, G);
  }
...
std::vector<Particle> gather_global_particles(const std::vector<Particle>& local_particles,
  ...
  MPI_Allgatherv(packed_local.data(), static_cast<int>(packed_local.size()), MPI_DOUBLE,
                 packed_global.data(), packed_counts.data(), packed_offsets.data(), MPI_DOUBLE,
                 MPI_COMM_WORLD);
...
int main(int argc, char** argv) {
  ...
  if ((mode != "allpairs" && mode != "barneshut") || global_particle_count <= 0 || steps < 0 ||
      dt <= 0.0 || theta <= 0.0 || snapshot_interval < 0) {
    print_usage_if_needed(rank);
    if (rank == 0) {
      std::cerr << "Error: mode must be allpairs or barneshut; require N > 0, steps >= 0, dt > 0, "
                   "theta > 0, snapshot_interval >= 0.\n";
    }
    MPI_Finalize();
    return 1;
  }
  ...
  MPI_Scatterv(rank == 0 ? packed_global_particles.data() : nullptr, packed_counts.data(),
               packed_offsets.data(), MPI_DOUBLE, packed_local_particles.data(),
               static_cast<int>(packed_local_particles.size()), MPI_DOUBLE, 0, MPI_COMM_WORLD);

  std::vector<Particle> local_particles = unpack_particles(packed_local_particles);

  if (rank == 0) {
    std::cout << "MPI initialized with " << size << " ranks.\n";
    std::cout << "Running mode = " << mode << ", N = " << global_particle_count
              << ", steps = " << steps << ", dt = " << dt << ", theta = " << theta
  ...
  global_particles = gather_global_particles(local_particles, global_particle_count, packed_counts,
                                             packed_offsets);
  ...
  for (int step = 1; step <= steps; ++step) {
    global_particles = gather_global_particles(local_particles, global_particle_count,
                                               packed_counts, packed_offsets);

    if (mode == "allpairs") {
      step_particles_from_global(local_particles, global_particles, dt);
    } else {
      step_particles_barnes_hut_from_global(local_particles, global_particles,
                                            particle_offsets[rank], dt, theta);
    }
  ...
  if (rank == 0) {
    ...
    const Vec3 momentum_change(final_momentum.x - initial_momentum.x,
                               final_momentum.y - initial_momentum.y,
                               final_momentum.z - initial_momentum.z);

    const double p_mag_change =
        std::sqrt(momentum_change.x * momentum_change.x + momentum_change.y * momentum_change.y +
    ...
    std::cout << "Momentum change: (" << momentum_change.x << ", " << momentum_change.y << ", "
              << momentum_change.z << ")\n";
    ...
    std::cout << "SUMMARY "
              << "mode=" << mode << " ranks=" << size << " N=" << global_particle_count
              << " steps=" << steps << " dt=" << dt << " theta=" << theta
              << " elapsed=" << max_elapsed
              << " avg_step=" << (steps > 0 ? max_elapsed / steps : 0.0)
              << " mass_change=" << mass_change << " px_change=" << momentum_change.x
              << " py_change=" << momentum_change.y << " pz_change=" << momentum_change.z
